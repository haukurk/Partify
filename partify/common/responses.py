...
def create_error_response(status, messages):
    """
    Create a response that returns status with a message error.
    :param status: error status string
    :param messages: message of the error
    :return: dictionary
    """
    return {"status": status,
            "message": messages}